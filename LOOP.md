...
`LOOP-FIGURES.md` — the figure-sourcing sweep: work `sources/figure-queue.md` (regenerated by
`scripts/build_figure_queue.py`) to embed each source's load-bearing chart in its entry per
EDITORIAL §3c. ≤4 figures per shift, every crop visually verified, editor review before merge.

`docs/publish-notification-protocol.md` — the notification handoff between the loop agent and
Hugh (Hermes). When the loop publishes pages to Ghost, it appends to
`sources/.publish-notify.json`; Hugh's cron reads that file and reports new/updated pages to
the team Slack channel. The loop agent's responsibility is documented in the protocol spec.
