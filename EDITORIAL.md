...
- **Re-embeds (added 2026-07-16):** a hosted figure may ALSO appear on an
  outcome/problem/benefit/concept page whose **headline claim** it directly evidences —
  same caption and credit, plus a link to the source's wiki entry. The research/books
  entry about the source is the canonical home and embeds first; one figure per page
  still. Placement candidates are listed in `sources/figure-queue.md`.
- **The loop:** this policy is executed by the figure-sourcing loop — see
  `LOOP-FIGURES.md` (runbook) and `sources/figure-queue.md` (the scored queue,
  regenerated by `scripts/build_figure_queue.py`).
