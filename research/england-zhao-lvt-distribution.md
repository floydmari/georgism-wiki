...
Crucially, England and Zhao do not treat this as a reason to reject the two-rate/LVT approach — the paper's own proposed fix is a uniform credit funded from the same revenue, and its design lesson is that **an unadorned shift toward land taxation is not automatically progressive, and its progressivity in practice depends on pairing it with a redistributive mechanism** (a uniform credit, a citizen's dividend, or an equivalent rebate) rather than relying on land-concentration alone. The same credit-based design was proposed decades later by the Canadian advocacy think tank Common Wealth Canada, whose [modelling of a national LVT](/wiki/cwc-distributional-impacts-lvt/) paired with a large flat refundable credit reports a similar pattern (LVT alone regressive by income; the credit leaving most households better off) — think-tank modelling rather than peer-reviewed replication of this study.
...
- **Research:** [Common Wealth Canada — Assessing the Distributional Impacts of a Land Value Tax](/wiki/cwc-distributional-impacts-lvt/) — a later think-tank modelling exercise proposing the same credit-based remedy for a national Canadian LVT; the organisation's own modelling, not an independent test of this paper's result.
...
*Sourcing note (2026-07-05):* the full 24-page Lincoln Institute working-paper PDF was read in full, confirming the parcel count (5,250 single-family homes), Zhao's affiliation (Ohio State University), the Dover/2002 scope, and the regressivity and credit-offset magnitudes quoted above. The published *National Tax Journal* version remains paywalled; the working paper is the same study and is the copy this page relies on.