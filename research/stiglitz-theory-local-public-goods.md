---
authors:
- Joseph Stiglitz
category: research
excerpt: The 1977 Joseph Stiglitz paper that coined the 'Henry George Theorem' — showing
  that under optimal conditions, land rents exactly fund optimal public-goods spending.
source_url: https://www.nber.org/
stub: false
subcategory: wiki-research-resources
supports_outcomes:
- public-goods-fundable-from-land-rent
tags:
- research
- stiglitz
- henry-george-theorem
- public-goods
- foundational
tier: core
title: The Theory of Local Public Goods
year: 1977
---

## Summary

This 1977 essay by [Joseph Stiglitz](/wiki/joseph-stiglitz/) is where the term **"Henry George Theorem"** was coined. It is the foundational theoretical statement that a single tax on land rent can finance public goods.

## The Result

Stiglitz showed that, for an optimally sized community providing public goods at the optimal level, the **aggregate land rent generated equals the optimal expenditure on those public goods**. Public goods raise the desirability of locations, which is capitalised into land rent (see [tax capitalization](/wiki/tax-capitalization/)); capturing that rent recovers exactly what the public spending created. He demonstrated this in a formal general-equilibrium setting, giving Henry George's 19th-century intuition a rigorous modern foundation.

## Relationship to Other Work

The result was elaborated the same era in [Arnott & Stiglitz (1979)](/wiki/arnott-stiglitz-henry-george-theorem/) and later extended to realistic "second-best" conditions by [Behrens et al. (2015)](/wiki/behrens-hgt-second-best/). Together these form the theoretical backbone of the [Henry George Theorem](/wiki/henry-george-theorem/).

## See Also

- [Henry George Theorem](/wiki/henry-george-theorem/) · [Joseph Stiglitz](/wiki/joseph-stiglitz/) · [Public goods can be funded from the land rent they create](/wiki/public-goods-fundable-from-land-rent/)

## Sources

1. Joseph E. Stiglitz (1977), "The Theory of Local Public Goods," in *The Economics of Public Services* (Macmillan). Related NBER work: [nber.org](https://www.nber.org/) — used for Stiglitz's statement of the Henry George Theorem, that under optimal city size aggregate land rents equal spending on local public goods.
2. Formalization: [Arnott & Stiglitz (1979) — wiki summary](/wiki/arnott-stiglitz-henry-george-theorem/) — used as a cross-reference for the contemporaneous formalization mentioned in §"Relationship to Other Work" above.