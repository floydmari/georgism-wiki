...
A note on provenance: the Myerson–Satterthwaite paper itself (source 1) was read and quoted verbatim this session, but the attribution to *Radical Markets* (source 2) rests on this wiki's existing discovery-report notes on the book rather than a fresh reading of the primary text — the exact footnote wording and page in Posner and Weyl's Chapter 1 have not been re-verified against the book directly, so that citation should be read as carried second-hand pending a page-level check by a future editor with the book in hand.
