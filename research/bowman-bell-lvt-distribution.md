...
A parallel qualification is proposed by the Canadian advocacy think tank Common Wealth Canada, whose own [distributional modelling](/wiki/cwc-distributional-impacts-lvt/) — a single, non-peer-reviewed exercise — reports that a national LVT alone would be regressive by current income unless paired with a flat refundable credit. Bowman and Bell's peer-reviewed finding is different in kind — it is not about needing a rebate, but about the **underlying incidence pattern varying by place** — but the upshot is the same: the progressivity outcome page's own caveat that "progressivity depends on design" should also be read as "and on local land-value/income structure," not only on policy design choices like exemptions or dividends.
...
- **Research:** [Common Wealth Canada — distributional impacts of LVT](/wiki/cwc-distributional-impacts-lvt/) — an advocacy think tank's modelling exercise in which LVT-alone incidence also came out regressive absent an added policy design element; the two point to different routes (place-dependence here; rebate-design there) by which "LVT is progressive" may need qualification.
