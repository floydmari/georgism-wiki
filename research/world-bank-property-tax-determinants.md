---
title: "Determinants of Property Tax Revenue: Lessons from Empirical Analysis"
category: research
tags: [research, property-tax, world-bank, global, revenue]
authors: [World Bank]
year: 2020
source_url: https://documents1.worldbank.org/curated/en/360151600192574693/pdf/Determinants-of-Property-Tax-Revenue-Lessons-from-Empirical-Analysis.pdf
tier: important
stub: false
excerpt: "A cross-country analysis of property-tax revenue performance across 128 countries, identifying what drives successful property and land taxation."
supports_outcomes: [property-tax-raises-welfare-developing]
---

## Summary

This 2020 World Bank policy research working paper (PRW 9399) analyses property-tax revenue across **128 countries**, asking why some collect far more than others from a tax economists widely favour.

## Key Findings

Property-tax revenue depends heavily on **administrative capacity** — quality of valuation/cadastre, collection systems, and enforcement — far more than on statutory rates. Richer countries with better cadastral infrastructure raise much more; many developing countries leave large potential uncollected. This complements the [IMF's assessment](/wiki/imf-taxing-immovable-property/): land and property taxes are efficient and under-used, and the binding constraint is **administration**, especially [assessment](/wiki/land-cannot-be-assessed/), not the economics.

## Bears On

- **Objection:** [Land value can't be assessed accurately](/wiki/land-cannot-be-assessed/)
- **Outcome:** [Property taxes raise welfare in developing countries](/wiki/property-tax-raises-welfare-developing/)

## Sources

1. World Bank (2020), "Determinants of Property Tax Revenue: Lessons from Empirical
   Analysis," Policy Research Working Paper 9399 — used for the cross-country evidence
   on what drives property-tax revenue (administration, valuation coverage, and
   institutional capacity, more than statutory rates).
   [PDF](https://documents1.worldbank.org/curated/en/360151600192574693/pdf/Determinants-of-Property-Tax-Revenue-Lessons-from-Empirical-Analysis.pdf)  ·  [📄 Local copy](https://pub-6311cf78f0ae499a804b562ce22dcfc2.r2.dev/sources/world-bank-property-tax-determinants.pdf)
